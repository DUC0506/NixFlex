...
import {getRecommendMovies} from "../store/action";
...
return(
  ...
  Want more inspiration? Browse our search results...
    ...
    <Card className="ProductItem" style={{ width: '18rem' }}  key={movie.id}>
      ...
      <Card.Body>
        <Card.Title className="ProductItemTitle" >{movie &&(movie.title||movie.name)}</Card.Title>
        
        <Button className="btnPlay" variant="primary" onClick={()=>{handleWatchMovie(movieid=movie.id, moviename=movie.title||movie.name)}}> <FaPlay/> Play</Button>
      </Card.Body>
    </Card>
